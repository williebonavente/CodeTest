...
\begin{algorithm}[hbt!]
    \caption{Bresenham Line Drawing Algorithm}\label{alg:bresenham_line}
    \KwData{Coordinates of two endpoints: $(x_1, y_1)$ and $(x_2, y_2)$}
    \KwResult{List of points on the line between $(x_1, y_1)$ and $(x_2, y_2)$}
    
    $\Delta x \gets x_2 - x_1$ \\
    $\Delta y \gets y_2 - y_1$ \\
    $dx2 \gets 2 \cdot \Delta x$ \\
    $dy2 \gets 2 \cdot \Delta y$ \\
    $slopeError \gets 0$ \\
    $y \gets y_1$ \\
    
    \ForEach{ $x$ in range $x_1$ to $x_2$ } {
        Plot point $(x, y)$ \Comment*[r]{Plot the current point}
        $slopeError \gets slopeError + dy2$ \\
        
        \If{$slopeError \geq \Delta x$}{
            $y \gets y + 1$ \Comment*[r]{Increment y for diagonal movement}
            $slopeError \gets slopeError - dx2$ \\
        }
    }
    \end{algorithm}
