...
\begin{algorithm}[hbt!]
    ...
    \If{$n \leq 1$}{
        \Return $A$ \Comment*[r]{Base case: Array with 1 or 0 elements is already sorted}
    }
    
    Select a pivot element $p$ from $A$ \Comment*[r]{Various pivot selection strategies exist}
    
    Partition $A$ into three subarrays: $L$ (elements less than $p$), $E$ (elements equal to $p$), and $G$ (elements greater than $p$)
    
    $sortedL \gets \text{QuickSort}(L)$ \Comment*[r]{Recursively sort the smaller elements}
    $sortedG \gets \text{QuickSort}(G)$ \Comment*[r]{Recursively sort the greater elements}
    
    \Return Concatenate($sortedL$, $E$, $sortedG$) \Comment*[r]{Concatenate the sorted subarrays}
    \end{algorithm}
    
%% Fibonacci Sequence %%
\begin{algorithm}[hbt!]
    \caption{Fibonacci Sequence}\label{alg:fibonacci}
    \KwData{An integer $n$}
    \KwResult{The $n$-th Fibonacci number}
    
    \If{$n \leq 1$}{
        \Return $n$ \Comment*[r]{Base case: Fibonacci(0) = 0, Fibonacci(1) = 1}
    }
    
    $prev \gets 0$ \\
    $current \gets 1$ \\
    \For{$i \gets 2$ \KwTo $n$}{
        $next \gets prev + current$ \\
        $prev \gets current$ \\
        $current \gets next$ \Comment*[r]{Update values for the next iteration}
    }
    
    \Return $current$ \Comment*[r]{The $n$-th Fibonacci number}
    \end{algorithm}

%% Harmonic Sequence %%
\begin{algorithm}[hbt!]
    \caption{Riemann Zeta Sequence}\label{alg:riemann_zeta}
    \KwData{An integer $n$}
    \KwResult{The $n$-th term in the Riemann Zeta sequence}
    
    $zeta \gets 0$ \\
    \For{$k \gets 1$ \KwTo $n$}{
        $zeta \gets zeta + \frac{1}{k^2}$ \Comment*[r]{Calculate the sum of the sequence}
    }
    
    \Return $zeta$ \Comment*[r]{The $n$-th term in the Riemann Zeta sequence}
    \end{algorithm}
